...
const heroListings = properties.slice(0, 3);
    ...
    <>
              ...
              <div key={property.slug} className={`collage-card c${index + 1}`}>
                <div className="facade">
                  ...
                  <div className="windows">
                    {Array.from({ length: 15 }, (_, i) => <i key={i} className={i % 2 === 0 ? "lit" : ""} />)}
                  </div>
                </div>
                <div className="info">
                  <b>{property.title}</b>
      ...
      <section className="section alt" id="projects">
        <div className="wrap">
          ...
          <div className="proj-row" role="list">
            {projects.map(([name, loc, price, units, pos]) => (
              <div className="proj-card" role="listitem" key={name}>
                <span className="badge"><span className="dot" />RERA verified</span>
                <h3>{name}</h3>
                <p className="loc">{loc}</p>
                <div className="proj-stats">
                  <div><span>Starting from</span><b>{price}</b></div>
                  <div><span>Unit types</span><b>{units}</b></div>
                  <div><span>Possession</span><b>{pos}</b></div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="section">
        <div className="wrap">
          ...
          <div className="steps">
            {[
              ["01", "Search and shortlist", "Filter by locality, budget and BHK. Compare up to four listings side by side."],
              ...
              ["04", "Move in, brokerage-free", "Close the deal directly. Applying for a loan? Do it in the same place, pre-filled from the listing."],
            ].map(([num, title, text]) => (
        ...
        <div className="wrap">
          <div className="emi">
            <div>
              ...
              <p>Adjust the loan amount and see your EMI instantly — no login needed. Ready to apply? Carry the numbers straight into a loan application.</p>
